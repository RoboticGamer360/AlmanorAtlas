...
router.get('/locations/food', (req, res) => {
  ...
  try {
    db = getDatabase();
    const dbResponse = db.prepare('SELECT * FROM food_locations WHERE pending = 0').all() as DB.FoodLocation[];
...
async function insertFoodLocation(request: API.NewFoodLocationRequest, pending: boolean): Promise<void> {
  let db: sqlite3.Database | undefined;
  try {
    ...
    db = getDatabase();
    db.prepare('INSERT INTO food_locations(name, description, address, color, image, pending) VALUES(?, ?, ?, ?, ?, ?)')
...
router.post('/contribute/locations/food', async (req, res) => {
  const body = req.body as API.NewFoodLocationRequest;
  try {
    await insertFoodLocation(body, false);
...
router.post('/locations/food', requireAuthorization, async (req, res) => {
  const body = req.body as API.NewFoodLocationRequest;
  try {
    await insertFoodLocation(body, false);
    res.status(201).json({ status: 'success' });
